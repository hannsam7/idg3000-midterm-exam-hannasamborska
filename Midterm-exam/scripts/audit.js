...
if (!fs.existsSync(lighthouseRunsDir)) fs.mkdirSync(lighthouseRunsDir, { recursive: true });
if (!fs.existsSync(hostingOutputDir)) fs.mkdirSync(hostingOutputDir, { recursive: true });

function median(arr) {
  const nums = arr.filter(v => typeof v === 'number' && !Number.isNaN(v)).sort((a, b) => a - b);
...
async function auditSite(url) {
    ...
    for (let i = 1; i <= NUM_RUNS; i++) {
      ...
      try {
        execSync(`npx lighthouse ${url} --output=json --output-path=${runFile} --chrome-flags="--headless" --quiet`, { stdio: 'inherit' });
  ...
  if (runs.length === 0) {
    ...
    const fcpVals = runs.map(r => getAuditNumeric(r, 'first-contentful-paint'));
    const lcpVals = runs.map(r => getAuditNumeric(r, 'largest-contentful-paint'));
    ...
    // Build a synthesized report using run 1 as base and overriding key fields
    ...
    if (!synthesized.categories.performance) synthesized.categories.performance = {};
    if (typeof medPerf === 'number') synthesized.categories.performance.score = medPerf;
    if (!synthesized.audits) synthesized.audits = {};
    if (!synthesized.audits['first-contentful-paint']) synthesized.audits['first-contentful-paint'] = {};
    if (!synthesized.audits['largest-contentful-paint']) synthesized.audits['largest-contentful-paint'] = {};
    if (!synthesized.audits['speed-index']) synthesized.audits['speed-index'] = {};
    if (!synthesized.audits['total-byte-weight']) synthesized.audits['total-byte-weight'] = {};
    if (!synthesized.audits['network-requests']) synthesized.audits['network-requests'] = { details: { items: [] } };
    if (!synthesized.audits['network-requests'].details) synthesized.audits['network-requests'].details = {};

    if (typeof medFcp === 'number') synthesized.audits['first-contentful-paint'].numericValue = medFcp;
    if (typeof medLcp === 'number') synthesized.audits['largest-contentful-paint'].numericValue = medLcp;
    if (typeof medSpeed === 'number') synthesized.audits['speed-index'].numericValue = medSpeed;
    if (typeof medBytes === 'number') synthesized.audits['total-byte-weight'].numericValue = medBytes;

    // Exporter only needs count; create placeholder array of that size
    synthesized.audits['network-requests'].details.items = Array.from({ length: medReqs || 0 }, () => ({}));

    const lighthouseFile = `${lighthouseOutputDir}${domain}.json`;
    fs.writeFileSync(lighthouseFile, JSON.stringify(synthesized, null, 2));
    console.log(`Saved synthesized median report for ${url} -> ${lighthouseFile}`);
  }
  ...
  try {
    ...
    if (greenRes.ok) {
      ...
      fs.writeFileSync(`${hostingOutputDir}${host}_greenweb.json`, JSON.stringify(greenJson, null, 2));
